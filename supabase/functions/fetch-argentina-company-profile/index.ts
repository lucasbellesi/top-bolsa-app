...
const CACHE_TTL_SECONDS = Number(
    Deno.env.get('ARGENTINA_COMPANY_PROFILE_CACHE_TTL_SECONDS') ?? '86400',
);
...
const readCache = async (ticker: string): Promise<CacheRow | null> => {
    const { data, error } = await supabase
        ...
        .select(
            'ticker,ticker_yahoo,market,company_name,description,sector,industry,market_cap,exchange,country,website,cached_at,updated_at',
        )
        .eq('ticker', ticker)
        .maybeSingle();
...
const buildProfileFromQuoteSummary = async (ticker: string): Promise<CompanyProfileData> => {
    ...
    const defaultKeyStatistics = isRecord(summary.defaultKeyStatistics)
        ? summary.defaultKeyStatistics
        : {};

    const companyName = getString(price.longName) || getString(price.shortName) || ticker;
...
const buildFallbackProfileFromQuote = async (ticker: string): Promise<CompanyProfileData> => {
    ...
    return {
        ...
        exchange:
            typeof quote.exchange === 'string' && quote.exchange.trim().length > 0
                ? quote.exchange.trim()
                : undefined,
        lastUpdatedAt: new Date().toISOString(),
    };
