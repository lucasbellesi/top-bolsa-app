...
const VALID_TIMEFRAMES: ReadonlySet<TimeframeType> = new Set([
    '1H',
    '1D',
    '1W',
    '1M',
...
const getLatestStockTimestamp = (stocks: StockData[]): string =>
    new Date(
        stocks.reduce((latest, stock) => {
            ...
            return typeof stockLatest === 'number' &&
                Number.isFinite(stockLatest) &&
                stockLatest > latest
                ? stockLatest
                : latest;
        }, Date.now()),
...
const fetchTicker = async (
    ...
    try {
        ...
        const percentChange =
            timeframe === '1D' && isFiniteNumber(quote.regularMarketChangePercent)
                ? quote.regularMarketChangePercent
                : timeframe === '1H'
                  ? (computedOneHourChange ?? 0)
                  : historical.length > 0
                    ? computePercentFromHistory(historical, priceCandidate)
                    : isFiniteNumber(quote.regularMarketChangePercent)
                      ? quote.regularMarketChangePercent
                      : 0;
