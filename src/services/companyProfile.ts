...
import type { CompanyProfileData, DataSourceType, MarketType } from '../types';
...
const fetchUSCompanyProfile = async (
    ...
    try {
        ...
        const profile: CompanyProfileData = {
            ...
            sector:
                typeof payload.Sector === 'string' && payload.Sector.trim().length > 0
                    ...
                    : undefined,
            industry:
                typeof payload.Industry === 'string' && payload.Industry.trim().length > 0
                    ? payload.Industry.trim()
                    : undefined,
            marketCap: parseNumericValue(payload.MarketCapitalization),
            exchange:
                typeof payload.Exchange === 'string' && payload.Exchange.trim().length > 0
        ...
        logger.warn(`US company profile fetch failed for ${normalizedTicker}:`, error);
...
const mapARProfileResponse = (
    ...
    return {
        ...
        market: 'AR',
        companyName:
            typeof baseProfile.companyName === 'string' && baseProfile.companyName.trim().length > 0
                ? baseProfile.companyName.trim()
                : fallbackCompanyName?.trim() || ticker.toUpperCase(),
        description:
            typeof baseProfile.description === 'string' && baseProfile.description.trim().length > 0
                ? baseProfile.description.trim()
                : undefined,
        sector:
            typeof baseProfile.sector === 'string' && baseProfile.sector.trim().length > 0
                ? baseProfile.sector.trim()
                : undefined,
        industry:
            typeof baseProfile.industry === 'string' && baseProfile.industry.trim().length > 0
                ? baseProfile.industry.trim()
                : undefined,
        marketCap: parseNumericValue(baseProfile.marketCap),
        exchange:
            typeof baseProfile.exchange === 'string' && baseProfile.exchange.trim().length > 0
                ? baseProfile.exchange.trim()
                : undefined,
        country:
            typeof baseProfile.country === 'string' && baseProfile.country.trim().length > 0
                ? baseProfile.country.trim()
                : undefined,
        website:
            typeof baseProfile.website === 'string' && baseProfile.website.trim().length > 0
                ? baseProfile.website.trim()
                : undefined,
        source: mapRemoteSourceToUiSource(payload.source, { allowMock: true }),
        lastUpdatedAt:
            typeof baseProfile.lastUpdatedAt === 'string' &&
            baseProfile.lastUpdatedAt.trim().length > 0
...
const fetchARCompanyProfile = async (
    ...
    try {
        const { data, error } = await supabase.functions.invoke<CompanyProfileResponse>(
            'fetch-argentina-company-profile',
            {
        ...
        logger.warn(`AR company profile fetch failed for ${normalizedTicker}:`, error);
        const cached = getCachedProfile('AR', normalizedTicker);
        if (cached) {
            return { ...cached, source: 'CACHE' };
        }
