...
describe('fetchARMarketGainers', () => {
    ...
    it('returns LIVE when edge function returns live stocks', async () => {
        ...
        expect(mockInvoke).toHaveBeenCalledWith('fetch-argentina-market', {
            body: { timeframe: '1D' },
        });
...
describe('fetchUSMarketGainers', () => {
                    ...
                    return {
                        json: async () => ({
                            quotes: [
                                {
                                    symbol: 'ALBT',
                                    longname: 'Avalon GloboCare Corp.',
                                    quoteType: 'EQUITY',
                                },
                            ],
                        }),
    ...
    it('avoids extra company name lookups for 1H so intraday data can still load', async () => {
        ...
        expect(
            fetchMock.mock.calls.some(([input]) => String(input).includes('finance/search')),
        ).toBe(false);
        expect(
            fetchMock.mock.calls.some(([input]) => String(input).includes('function=OVERVIEW')),
        ).toBe(false);
    });
    ...
    it('uses daily history for 1D US rankings', async () => {
        vi.spyOn(global, 'fetch').mockImplementation(async (input: RequestInfo | URL) => {
            ...
            if (url.includes('function=TOP_GAINERS_LOSERS')) {
                return {
                    json: async () => ({
                        top_gainers: [{ ticker: 'AAPL', price: '200', change_percentage: '1.2%' }],
                    }),
                } as Response;
            }
